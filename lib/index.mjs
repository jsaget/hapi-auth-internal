...
const register = async (server, options = { saltRounds: 10 }) => {
  ...
  server.route({
    ...
    handler: async (request, h) => {
      ...
      await server.methods[options.methods.addUserIfNotExists]({ firstname, lastname, email, scope: ['user'], providers: { internal: {name: 'internal', email, password: passwd }} })
  ...
  server.route({
    ...
    handler: async (request, h) => {
      ...
      await server.methods[options.methods.resetUserPassword]({ resetToken, password })
